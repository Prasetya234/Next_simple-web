...
export async function DeleteOrder(reqUrl: string, req: number) {
  try {
    const res =  await axios.delete(`${api}` + `${reqUrl}/` + req).then(res => res.data.content)
...
export async function CreateOrder(req: object, type: object) {
  try {
    const res =  await axios.post(`${api}/order`, req, type).then(res => res.data.content)
...
export async function UpdateOrder(id: number, req: object, type: object) {
  try {
    const res = await axios.put(api + "order/" + id, req, type).then(res => res.data.content)
